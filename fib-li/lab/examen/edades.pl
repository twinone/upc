...
range(R) :- ageRange(R,_,_).
rangeOf(Age, Range) :- ageRange(Range, Min, Max), between(Min, Max, Age).
...
%   2. involved-R-T means: "range R is involved in task T"
...
everyTaskAtMostTwoAgeRanges :-
    task(T),
    findall(involved-R-T, range(R), L),
    atMost(2, L),
    fail.
everyTaskAtMostTwoAgeRanges.
...
linkRangesWithTasks :-
    ...
    % a gangster is either not involved with a task or at least at one hour he will do that task
    expressOr(involved-R-T, Lits),
    fail.
linkRangesWithTasks.
...
displaySol(M):- nl,nl,displayRanges(M),!.

displayRanges(M) :-
    task(T),
    write('Task '), write(T), write(': '),
    displayRanges(M, T), fail.
displayRanges(_).

displayRanges(M, T) :-
    member(involved-R-T, M),
    write(R), write(' '), fail.
displayRanges(_,_):-nl.
