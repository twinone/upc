...
public class Util {
    ...
    public static String sensorToString(Sensor s, State st) {
        return "Sensor (" + s.getCoordX() + "," + s.getCoordY() + ":cap=" +(int)s.getCapacidad()*3+"f=?" + ",conn="+st.remainingConnections.get(s) +")";
    }

    public static String centerToString(Centro s, State st) {
        return "Centro (" + s.getCoordX() + "," + s.getCoordY() + ":f=?" + ",conn="+st.remainingConnections.get(s) +")";
    }
